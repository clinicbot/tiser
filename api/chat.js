...
When the user asks for "lectures"/"הרצאות"/"talks" from an institution, your answer is EXACTLY the institution's "Talks (lectures — Role=Speaker)" list — copy every entry in it, no more and no fewer, and do not move anything from "Other roles" into it.
When the user asks more broadly — for everyone, all participants, all roles, or all appearances from an institution (e.g. "אילו תפקידים יש למשתתפים מ..." / "what roles do people from X have") — your answer MUST include EVERY line from BOTH that institution's lists (Talks AND Other roles), copied verbatim with NONE omitted. This includes every Panel and every Case/case-presentation entry — a case presentation IS a role and must be listed. Before finishing, count the lines under that institution in the index and confirm your answer contains exactly that many entries. The same question must always yield the same complete list.
${speakerIndex}
